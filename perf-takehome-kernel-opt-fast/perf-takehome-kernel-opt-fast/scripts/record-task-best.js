...
function candidateFromValidation(value) {
	const cycles = value.metrics?.cycles ?? value.metrics?.median_ms ?? null;
	return {
...
function candidateSolutionSnapshot(taskDirRel, row) {
	...
	// The artifact path may already be repo-relative and start with taskDirRel (e.g.
	// "tasks/kernel_opt/runs/<ts>/perf_takehome.py"). Do NOT prepend taskDirRel again in that case,
	// or the snapshot path doubles to "tasks/kernel_opt/tasks/kernel_opt/runs/...".
	const normalizedDir = normalizeRel(artifactDir);
	if (normalizedDir === taskDirRel || normalizedDir.startsWith(`${taskDirRel}/`)) {
		return normalizeRel(path.join(normalizedDir, solution));
	}
