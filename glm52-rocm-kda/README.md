...
This directory contains two related GLM-5.2 ROCm KDA artifacts:

- `glm52-rocm-kda.omhflow` and `glm52-rocm-kda-local.omhflow`: OMH workflow
  artifacts for the campaign at:

```text
/home/lichangye/glm52-rocm-kda
```

- `KDA_PILOT_WORKFLOW.md`: the Humanize KDA-Pilot runbook distilled from the
  `/home/lichangye/kernel-harness-amd` Claude/Codex RLCR optimization loops.
  It records the frozen MI300X authority, no-regression/maximize contract,
  ROCm environment rules, model split, latest accepted results, and failure
  handling lessons.

The OMH flow implements the initial 3 worker + 2 searcher topology:
