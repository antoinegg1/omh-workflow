...
# Determine automation flag based on environment variable. Default to bypass on
# Linux hosts where Codex's bwrap sandbox cannot create mount namespaces.
...
CODEX_MAX_ATTEMPTS="${HUMANIZE_CODEX_EXEC_MAX_ATTEMPTS:-6}"
CODEX_BACKOFF_INITIAL="${HUMANIZE_CODEX_EXEC_BACKOFF_INITIAL:-10}"
CODEX_BACKOFF_MAX="${HUMANIZE_CODEX_EXEC_BACKOFF_MAX:-300}"
if [[ ! "$CODEX_MAX_ATTEMPTS" =~ ^[1-9][0-9]*$ ]]; then
    CODEX_MAX_ATTEMPTS=6
fi
if [[ ! "$CODEX_BACKOFF_INITIAL" =~ ^[1-9][0-9]*$ ]]; then
    CODEX_BACKOFF_INITIAL=10
fi
if [[ ! "$CODEX_BACKOFF_MAX" =~ ^[1-9][0-9]*$ ]]; then
    CODEX_BACKOFF_MAX=300
fi
...

{
    ...
    echo "# Max attempts: $CODEX_MAX_ATTEMPTS"
    echo "# Backoff: ${CODEX_BACKOFF_INITIAL}s exponential, capped at ${CODEX_BACKOFF_MAX}s"
    echo ""
...
CODEX_EXIT_CODE=0
CODEX_ATTEMPT=1
CODEX_BACKOFF="$CODEX_BACKOFF_INITIAL"
: > "$CODEX_STDOUT_FILE"
: > "$CODEX_STDERR_FILE"
while [[ "$CODEX_ATTEMPT" -le "$CODEX_MAX_ATTEMPTS" ]]; do
    ATTEMPT_STDOUT_FILE="$CACHE_DIR/codex-run-attempt-${CODEX_ATTEMPT}.out"
    ATTEMPT_STDERR_FILE="$CACHE_DIR/codex-run-attempt-${CODEX_ATTEMPT}.log"
    echo "ask-codex: attempt ${CODEX_ATTEMPT}/${CODEX_MAX_ATTEMPTS}" >&2

    CODEX_EXIT_CODE=0
    printf '%s' "$QUESTION" | run_with_timeout "$CODEX_TIMEOUT" codex exec "${CODEX_EXEC_ARGS[@]}" - \
        > "$ATTEMPT_STDOUT_FILE" 2> "$ATTEMPT_STDERR_FILE" || CODEX_EXIT_CODE=$?
    cat "$ATTEMPT_STDOUT_FILE" > "$CODEX_STDOUT_FILE"
    cat "$ATTEMPT_STDERR_FILE" > "$CODEX_STDERR_FILE"
    ...
    echo "ask-codex: attempt ${CODEX_ATTEMPT} failed with exit_code=$CODEX_EXIT_CODE; retrying in ${CODEX_BACKOFF}s" >&2
    sleep "$CODEX_BACKOFF"
    CODEX_BACKOFF=$((CODEX_BACKOFF * 2))
    if [[ "$CODEX_BACKOFF" -gt "$CODEX_BACKOFF_MAX" ]]; then
        CODEX_BACKOFF="$CODEX_BACKOFF_MAX"
    fi
