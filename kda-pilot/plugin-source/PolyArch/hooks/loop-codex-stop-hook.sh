...
if [[ "${HUMANIZE_CODEX_BYPASS_SANDBOX:-true}" == "false" ]] || [[ "${HUMANIZE_CODEX_BYPASS_SANDBOX:-true}" == "0" ]]; then
    CODEX_AUTO_FLAG="--full-auto"
elif [[ "${HUMANIZE_CODEX_BYPASS_SANDBOX:-true}" == "true" ]] || [[ "${HUMANIZE_CODEX_BYPASS_SANDBOX:-true}" == "1" ]]; then
    CODEX_AUTO_FLAG="--dangerously-bypass-approvals-and-sandbox"
fi
CODEX_EXEC_ARGS+=("$CODEX_AUTO_FLAG" "-C" "$PROJECT_ROOT")

# codex review accepts sandbox/approval controls only as top-level Codex
# options, before the review subcommand. Keep the default aligned with
# codex exec so Linux hosts with broken bwrap can still review diffs.
CODEX_REVIEW_GLOBAL_ARGS=()
if [[ "$CODEX_AUTO_FLAG" == "--dangerously-bypass-approvals-and-sandbox" ]]; then
    CODEX_REVIEW_GLOBAL_ARGS+=("$CODEX_AUTO_FLAG")
elif [[ "$CODEX_AUTO_FLAG" == "--full-auto" ]]; then
    CODEX_REVIEW_GLOBAL_ARGS+=("-s" "danger-full-access" "-a" "never")
fi

# Build Codex command arguments for codex review
CODEX_REVIEW_ARGS=(
    "-c" "model=${CODEX_REVIEW_MODEL}"
    "-c" "review_model=${CODEX_REVIEW_MODEL}"
...
# Codex review can fail transiently at the provider layer even after the local
# review process starts correctly. Retry non-zero review exits before blocking
# the loop. These are total attempts, including the first try.
CODEX_REVIEW_MAX_ATTEMPTS="${HUMANIZE_CODEX_REVIEW_MAX_ATTEMPTS:-6}"
CODEX_REVIEW_BACKOFF_INITIAL="${HUMANIZE_CODEX_REVIEW_BACKOFF_INITIAL:-10}"
CODEX_REVIEW_BACKOFF_MAX="${HUMANIZE_CODEX_REVIEW_BACKOFF_MAX:-300}"
if [[ ! "$CODEX_REVIEW_MAX_ATTEMPTS" =~ ^[1-9][0-9]*$ ]]; then
    CODEX_REVIEW_MAX_ATTEMPTS=6
fi
if [[ ! "$CODEX_REVIEW_BACKOFF_INITIAL" =~ ^[1-9][0-9]*$ ]]; then
    CODEX_REVIEW_BACKOFF_INITIAL=10
fi
if [[ ! "$CODEX_REVIEW_BACKOFF_MAX" =~ ^[1-9][0-9]*$ ]]; then
    CODEX_REVIEW_BACKOFF_MAX=300
fi
CODEX_EXEC_MAX_ATTEMPTS="${HUMANIZE_CODEX_EXEC_MAX_ATTEMPTS:-$CODEX_REVIEW_MAX_ATTEMPTS}"
CODEX_EXEC_BACKOFF_INITIAL="${HUMANIZE_CODEX_EXEC_BACKOFF_INITIAL:-$CODEX_REVIEW_BACKOFF_INITIAL}"
CODEX_EXEC_BACKOFF_MAX="${HUMANIZE_CODEX_EXEC_BACKOFF_MAX:-$CODEX_REVIEW_BACKOFF_MAX}"
if [[ ! "$CODEX_EXEC_MAX_ATTEMPTS" =~ ^[1-9][0-9]*$ ]]; then
    CODEX_EXEC_MAX_ATTEMPTS="$CODEX_REVIEW_MAX_ATTEMPTS"
fi
if [[ ! "$CODEX_EXEC_BACKOFF_INITIAL" =~ ^[1-9][0-9]*$ ]]; then
    CODEX_EXEC_BACKOFF_INITIAL="$CODEX_REVIEW_BACKOFF_INITIAL"
fi
if [[ ! "$CODEX_EXEC_BACKOFF_MAX" =~ ^[1-9][0-9]*$ ]]; then
    CODEX_EXEC_BACKOFF_MAX="$CODEX_REVIEW_BACKOFF_MAX"
fi
...
"
...

    {
        ...
        echo "# Max attempts: $CODEX_REVIEW_MAX_ATTEMPTS"
        echo "# Backoff: ${CODEX_REVIEW_BACKOFF_INITIAL}s exponential, capped at ${CODEX_REVIEW_BACKOFF_MAX}s"
        echo ""
        echo "codex ${CODEX_REVIEW_GLOBAL_ARGS[*]} review ${CODEX_DISABLE_HOOKS_ARGS[*]} --base $review_base ${CODEX_REVIEW_ARGS[*]}"
    ...
    local attempt=1
    local backoff="$CODEX_REVIEW_BACKOFF_INITIAL"
    while [[ "$attempt" -le "$CODEX_REVIEW_MAX_ATTEMPTS" ]]; do
        local attempt_log="$CACHE_DIR/round-${round}-codex-review-attempt-${attempt}.log"
        echo "Codex review attempt ${attempt}/${CODEX_REVIEW_MAX_ATTEMPTS}..." >&2

        CODEX_REVIEW_EXIT_CODE=0
        (cd "$PROJECT_ROOT" && run_with_timeout "$CODEX_TIMEOUT" codex "${CODEX_REVIEW_GLOBAL_ARGS[@]}" review "${CODEX_DISABLE_HOOKS_ARGS[@]}" --base "$review_base" "${CODEX_REVIEW_ARGS[@]}") \
            > "$attempt_log" 2>&1 || CODEX_REVIEW_EXIT_CODE=$?
        if [[ -s "$attempt_log" ]]; then
            {
                echo "===== Codex review attempt ${attempt}/${CODEX_REVIEW_MAX_ATTEMPTS} at $(date -u +%Y-%m-%dT%H:%M:%SZ) ====="
                echo
                cat "$attempt_log"
            } >> "$CODEX_REVIEW_LOG_FILE"
        fi

        echo "Codex review attempt ${attempt} exit code: $CODEX_REVIEW_EXIT_CODE" >&2
        if [[ "$CODEX_REVIEW_EXIT_CODE" -eq 0 ]]; then
            break
        fi
        if [[ "$attempt" -ge "$CODEX_REVIEW_MAX_ATTEMPTS" ]]; then
            break
        fi

        echo "Codex review failed; retrying in ${backoff}s..." >&2
        sleep "$backoff"
        backoff=$((backoff * 2))
        if [[ "$backoff" -gt "$CODEX_REVIEW_BACKOFF_MAX" ]]; then
            backoff="$CODEX_REVIEW_BACKOFF_MAX"
        fi
        attempt=$((attempt + 1))
    done
...
CODEX_PROMPT_CONTENT=$(cat "$REVIEW_PROMPT_FILE")
{
    ...
    echo "# Working directory: $PROJECT_ROOT"
    echo "# Timeout: $CODEX_TIMEOUT seconds"
    echo "# Max attempts: $CODEX_EXEC_MAX_ATTEMPTS"
    echo "# Backoff: ${CODEX_EXEC_BACKOFF_INITIAL}s exponential, capped at ${CODEX_EXEC_BACKOFF_MAX}s"
    echo ""
...
CODEX_EXEC_ATTEMPT=1
CODEX_EXEC_BACKOFF="$CODEX_EXEC_BACKOFF_INITIAL"
while [[ "$CODEX_EXEC_ATTEMPT" -le "$CODEX_EXEC_MAX_ATTEMPTS" ]]; do
    CODEX_ATTEMPT_STDOUT_FILE="$CACHE_DIR/round-${CURRENT_ROUND}-codex-run-attempt-${CODEX_EXEC_ATTEMPT}.out"
    CODEX_ATTEMPT_STDERR_FILE="$CACHE_DIR/round-${CURRENT_ROUND}-codex-run-attempt-${CODEX_EXEC_ATTEMPT}.log"
    echo "Codex summary review attempt ${CODEX_EXEC_ATTEMPT}/${CODEX_EXEC_MAX_ATTEMPTS}..." >&2
    ...
    printf '%s' "$CODEX_PROMPT_CONTENT" | run_with_timeout "$CODEX_TIMEOUT" codex exec "${CODEX_DISABLE_HOOKS_ARGS[@]}" "${CODEX_EXEC_ARGS[@]}" - \
        > "$CODEX_ATTEMPT_STDOUT_FILE" 2> "$CODEX_ATTEMPT_STDERR_FILE" || CODEX_EXIT_CODE=$?
    cat "$CODEX_ATTEMPT_STDOUT_FILE" > "$CODEX_STDOUT_FILE"
    cat "$CODEX_ATTEMPT_STDERR_FILE" > "$CODEX_STDERR_FILE"

    echo "Codex summary review attempt ${CODEX_EXEC_ATTEMPT} exit code: $CODEX_EXIT_CODE" >&2
    if [[ "$CODEX_EXIT_CODE" -eq 0 ]]; then
        break
    fi
    if [[ "$CODEX_EXEC_ATTEMPT" -ge "$CODEX_EXEC_MAX_ATTEMPTS" ]]; then
        break
    fi

    echo "Codex summary review failed; retrying in ${CODEX_EXEC_BACKOFF}s..." >&2
    sleep "$CODEX_EXEC_BACKOFF"
    CODEX_EXEC_BACKOFF=$((CODEX_EXEC_BACKOFF * 2))
    if [[ "$CODEX_EXEC_BACKOFF" -gt "$CODEX_EXEC_BACKOFF_MAX" ]]; then
        CODEX_EXEC_BACKOFF="$CODEX_EXEC_BACKOFF_MAX"
    fi
    CODEX_EXEC_ATTEMPT=$((CODEX_EXEC_ATTEMPT + 1))
done
...
# Codex exec often returns the review on stdout instead; accept stdout when the
# workspace result is missing or empty.
if [[ ! -s "$REVIEW_RESULT_FILE" && -s "$CODEX_STDOUT_FILE" ]]; then
