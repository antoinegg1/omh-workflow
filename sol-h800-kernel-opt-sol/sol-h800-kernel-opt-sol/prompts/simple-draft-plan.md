...
If `taskContext.wiki_excerpt` is populated, it holds the latest research findings
maintained by the wiki-search lane for this operator (web-sourced techniques,
implementation directions, correctness/reward-hack risks). Treat it as advisory
evidence: prefer its concrete, H800/SM90-applicable directions, but verify claims
against the actual task before committing to them. Ignore Blackwell-only items.
