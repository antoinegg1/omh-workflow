...
if (shouldProfile) {
	...
	const command = [
		...
		`HELPERS=${process.env.NCU_REPORT_HELPERS_DIR ?? "<ncu-report-skill>/helpers"}`,
		`PROFILE_RUN_DIR=${path.relative(root, profileDir)}`,
		`python3 ${process.env.H800_RUN_TASK_SCRIPT ?? "<kernel-opt>/scripts/run_h800_task.py"} ${taskContext.task_dir} --solution-name ${validation.solution ?? "solution.json"} --candidate ${candidate}`,
	].join("\n");
